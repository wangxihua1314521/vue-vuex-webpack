<template>
	<div id="clanNews" class="view view-main" v-cloak>
							...
							<ul class="blog_list">
								<li v-for="hitem in hotReplyList">
									...
									<div class="comment-content">
										...
										<div class="comment-mid">{{hitem.replyContent}}</div>
										<div class="comment-down">
											<div class="del" @click="Delete(hitem)" v-if="hitem.isDel"><span>删除</span></div>
       ...
       <!--写评论-->
          ...
          <div class="toolbar-inner">
            <textarea placeholder="说点什么..." v-model="commentContent" @keyup="input($event)"></textarea><a href="#" class="link" @click="send_comment">发送</a>
          </div>
        </div>-->
       <!--分享窗口-->
	...
	export default {
		...
		methods:{
			...
			load:function(){
				...
				//http:127.0.0.1:8020/hxgm-web-rebuild/template/app/clanNews.html?newsId=43966e9e-7252-4286-bcf8-4ac85c1d2140&Authorization=NzUxOjNmN2E2ZjA3ODg0MDJkYzA5YjY3NjQ3NjM1MmMxZmFl&userId=40b321dd-ffa5-4be2-bbe8-f2880bb94b52
...
//			                   	console.log("查询 新闻评论 返回："+JSON.stringify(response.data.data));
	                   	...
	                   	that.hotReplyList=response.data.data.HotReplyList;
					      ...
					      $$('.infinite-scroll-preloader').remove();
	                   	...
	                   	if(!hotReply || hotReply.length==0){
                   			...
                   			for(var i=0;i<hotReply.length;i++){
                   				if(response.data.data.HotReplyList[i].replyPersonUrl.length == 0){
                   					response.data.data.HotReplyList[i].replyPersonUrl.push("images/img/headImg.png");
                   				}else{
                   					response.data.data.HotReplyList[i].replyPersonUrl=response.data.data.HotReplyList[i].replyPersonUrl[0].thumbnailUrlSmall;
                   				}
			...
			//热门评论点赞
			hotprised:function(hitem){
				var param = {type:'009'};
				var url = '/reply-prise/prise/'+hitem.replyId;
				var callback = {
                success:function(response){
//		                   	console.log("热门评论点赞返回：");console.log(response);
				 ...
				 http_site_app(url,'post',param,callback);
			},
			//点赞
			prised:function(item){
				...
				var callback = {
                success:function(response){
//		                   	console.log("评论点赞返回：");console.log(response);
...
//			                   	console.log("提交评论返回：");console.log(response);
	                   	...
	                   	if(response.data.code == 200){
							...
							that.replyList= [];
							that.HotReplyList=[];
							console.log("userId="+that.param.userId)
			...
			Delete:function(item){
	        	...
	        	var callback = {
	                success:function(response){
	                	clan_news.replyList=[];
	                   	console.log("删除数据 返回："+JSON.stringify(response.data));
	                   	that.currPage = 1;
	                   	myApp.detachInfiniteScroll($$('.infinite-scroll'));
	                   	clan_news.blogList();
	                }
	            }
	            http_site_app(url,'DELETE','',callback);
			},
			//视频新闻 点击详情
			lookContain:function(){
...
//						window.location.href="replayDetail.html?replyId="+obj+"&Authorization="+user_authorId+"&userId="+that.param.userId;
	...
	$$('.infinite-scroll').on('infinite', function () {
	  if (loading) return
	  loading = true;
	  clan_news.currPage = clan_news.currPage + 1;
	  clan_news.blogList();
	});
	//点击图片放大
