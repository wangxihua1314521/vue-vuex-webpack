...
const sendAjax = (urlBase, url, method, params, success, error) => {
  ...
  .catch(errorMsg => {
    if (errorMsg.config) {
      swal('网络连接失败,请稍后重试', '亲，连接服务器失败了哟~','error')
  ...
  for (let sys of sysList) {
    ...
    switch (sys.contextPath) {
      ...
      case '/hxgm-site-admin-web':
        ...
        break
        // 官网总后台
      case '/hxgm-official-admin-web':
        sysMap.set('official-admin', urlBase)
        break
        //消息中心
      case '/hxgm-message-web':
        sysMap.set('message', urlBase)
        break
        //根脉商城
      case '/hxgm-mall-web':
        sysMap.set('mall', urlBase)
        break
        //控制中心
      case '/hxgm-job-web':
        sysMap.set('job', urlBase)
        break
        //报表系统
      case '/hxgm-report-web':
        sysMap.set('report', urlBase)
        break
        //官网
      case '/hxgm-message-web':
        sysMap.set('message', urlBase)
